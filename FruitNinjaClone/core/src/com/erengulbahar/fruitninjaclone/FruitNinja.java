package com.erengulbahar.fruitninjaclone;
...
public class FruitNinja extends ApplicationAdapter implements InputProcessor
{
	...
	public void render ()
	{
		...
		if(lives >= 0)
		{
			...
			genSpeed = genSpeed - deltaTime * 0.015f;
			...
			{
				genCounter = genCounter - deltaTime;
			}

			for(int i=0;i<lives;i++)
			{
				batch.draw(apple,i*30f+60f,Gdx.graphics.getHeight()-50f,30f,30f);
			}
			...
			for(Fruit fruit : fruitArray)
			{
				if(fruit.outOfScreen())
				{
					...
					if(fruit.type == Fruit.Type.REGULAR)
					{
						lives--;
						break;
					}
		...
		if(lives <= 0)
		{
			font.draw(batch,"Cut to play",Gdx.graphics.getWidth()/2,Gdx.graphics.getHeight()/2);
		}
	...
	private void addItem()
	{
		...
		Fruit item = new Fruit(new Vector2(pos,-Fruit.radius),new Vector2((Gdx.graphics.getWidth() * 0.5f - pos) * (random.nextFloat() - 0.2f),Gdx.graphics.getHeight()*0.5f));
